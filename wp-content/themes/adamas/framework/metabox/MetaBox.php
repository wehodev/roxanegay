...
class WPAlchemy_MetaBox
{
    ...
    public function __construct($arr)
    {
        ...
        if (is_array($arr))
        {
            ...
            // Save gallery
            add_action( 'wp_ajax_gallery_meta_box_ajax', array( &$this, 'gallery_meta_box_ajax' ) );
     ...
     */
    function gallery_meta_box_ajax() {

        // Check whether form is submitted properly
        if ( ! isset( $_POST['ids'] ) || ! isset( $_POST['meta_id'] ) || ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'wpus-gallery-ajax' ) ) {
            return;
        }
        ...
        update_post_meta( $_POST['post_id'], $_POST['meta_id'], $ids );

        $thumbs = explode( ',', $ids );
        $thumbnail_output = '';
        foreach( $thumbs as $thumb ) {
            $thumbnail_output .= wp_get_attachment_image( $thumb, array( 114, 114 ) ) ;
        }
