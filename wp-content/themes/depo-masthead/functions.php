...
<?php if ( !is_dynamic_sidebar() ) { ?><h2>Etc</h2><?php } ?>
...
register_sidebar_widget('DePo About', 'depo_about_widget');
...
register_sidebar_widget('DePo Archives & Categories', 'depo_archives_widget');
...
register_sidebar_widget('RSS Link', 'rss_widget');
function rss_widget() { ?>
	<li id="rss_link" class="widget widget_rss"><a href="<?php bloginfo('rss2_url'); ?>">RSS Feed</a></li>
<?php 
}
...
function depo_add_theme_page() {
	...
	add_theme_page(__('DePo Options'), __('DePo Options'), 'edit_themes', basename(__FILE__), 'depo_theme_page');
}
function depo_theme_page() {
	...
	<form method="post" action="<?php echo attribute_escape($_SERVER['REQUEST_URI']); ?>">
		...
		<p><label for="author-name"><?php _e('Author Box Text:'); ?></label> <input type="text" name="author-name" value="<?php echo get_option('depo-author-name'); ?>" id="author-name" /> <small>Leaving this field blank will insert the blog author's name.</small></p>
		<p><input type="hidden" name="action" value="save" /> <input type="submit" name="submit" value="<?php echo attribute_escape(__('Submit')); ?>" id="submit" /></p>
