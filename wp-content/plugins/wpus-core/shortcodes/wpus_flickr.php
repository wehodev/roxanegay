...
function adamas_flickr_shortcode( $atts, $content = null ) {
    ...
    $icon_html    = $icon_appearance    ? '<i class="adamas-icon-flickr ' . $icon_appearance . '"></i>' : '';
    $photos_html  = '';
    $build_css    = '';
    ...
    if ( $overlay_css ) {
        $build_css .= ".wpus-flickr.{$unique_id} .wpus-overlay{{$overlay_css}}";
    }

    // Style: Icon color
    if ( $icon_css = AdamasHelper::get_style( array( 'color' => $icon_color ) ) ) {
        $build_css .= ".wpus-flickr.{$unique_id} .adamas-icon-flickr{{$icon_css}}";
    }
    ...
    // Get photos
    $flickr_array = adamas_get_flickr( $username, $number );

    if ( is_wp_error( $flickr_array ) ) {
        return $flickr_array->get_error_message();
    } 

    // Photos markup
    foreach ( $flickr_array as $item ) {
        if ( 'wpus-grid' == $type ) $photos_html .= '<div class="wpus-grid-item">';

            $photos_html .= '<figure class="wpus-rollover">';
                $photos_html .= '<a href="' . esc_url( 'true' == $lighbox ? $item['large'] : $item['url'] ) . '" target="_blank">';
                    $photos_html .= '<span class="image-background" data-hover="' . esc_attr( $image_hover ) . '" style="background-image:url(' . esc_url( $item['medium'] ) . ')"></span>';
...
add_shortcode( 'wpus_flickr', 'adamas_flickr_shortcode' );