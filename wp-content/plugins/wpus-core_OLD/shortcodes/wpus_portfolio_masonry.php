...
function adamas_portfolio_masonry_shortcode( $atts, $content = null ) {
    ...
    if ( $the_query->have_posts() ) : while ( $the_query->have_posts() ) : $the_query->the_post();
        ...
        // Get item size
        if ( 'default' == $masonry_style ) {
            $item_size = get_post_meta( $post->ID, 'adamas_portfolio_masonry_size', true );
        } else {
            $item_size = WpusVcHelper::get_masonry_size( $i, $masonry_style );
        }
        ...
        if ( $image_hover_id ) {
            $image_hover_src = wp_get_attachment_image_src( $image_hover_id, 'full', true );
            $image_hover_src = '<span class="image-background" data-class="image-hover" data-hover="' . WpusVcHelper::get_hover_image_class( $image_hover ) . '" style="background-image:url(' . $image_hover_src[0] . ')"></span>';
        }
        ...
        $project_html .= '<div' . $project_class . '>';
            $project_html .= '<a href="' . get_permalink() . '" class="wpus-rollover">';
                ...
                $project_html .= '<span class="image-background" data-hover="' . $image_hover . '" style="background-image:url(' . $thumbnail[0] . ')"></span>';
                $project_html .= $image_hover_src;
                $project_html .= $overlay;
                $project_html .= $title_html;
...
add_shortcode( 'wpus_portfolio_masonry', 'adamas_portfolio_masonry_shortcode' );