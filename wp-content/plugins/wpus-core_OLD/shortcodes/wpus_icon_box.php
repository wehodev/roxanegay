...
function adamas_icon_box_shortcode( $atts, $content = null ) {
    ...
    extract( shortcode_atts( array(
        ...
        'icon_slicons'            => 'sl-user-female',
        'style'                   => 'style-bg',
        'position'                => 'wpus-icon-float-left',
        'icon_size'               => '',
        'icon_padding'            => '',
        'icon_margin_bottom'      => '',
        'icon_margin_left'        => '',
    ...
    if ( $icon_animation_type ) {
        $icon_attr = AdamasHelper::get_html_attributes( array(
            'wow-delay'    => AdamasHelper::sanitize_animation( $icon_animation_delay ),
            'wow-duration' => AdamasHelper::sanitize_animation( $icon_animation_duration ),
        ) );
    }

    // Style: Wrap
    if ( $wrap_css = AdamasHelper::get_vc_style( $css ) ) {
        $build_css .= ".wpus-icon-box.{$unique_id}{{$wrap_css}}";
    }
    ...
    $regular_css = AdamasHelper::get_style( array(
        ...
        'margin_left'      => $icon_margin_left,
        'margin_right'     => $icon_margin_right,
    ...
    if ( $regular_css ) {
        $build_css .= ".wpus-icon-box.{$unique_id} .wpus-icon-wrap{{$regular_css}}";
    }
    ...
    if ( $hover_css ) {
        $build_css .= ".wpus-icon-box.{$unique_id}:hover .wpus-icon-wrap{{$hover_css}}";
    }
    ...
    switch ( $link_type ) {
        case 'link':
            $link_html = WpusVcHelper::build_link( $link );
            break;
        case 'image':
    ...
    $icon_class = AdamasHelper::get_html_class( $icon_class );

    // Shortcode
    $output = '<div' . $wrap_id . $wrap_class . $wrap_attr . '>';
        $output .= '<div' . $icon_class . $icon_attr . '>' . $icon . $link_html . '</div>';
        $output .= '<div class="wpus-icon-content">' . wpb_js_remove_wpautop( $content ) . '</div>';
    $output .= '</div>'; 
...
add_shortcode( 'wpus_icon_box', 'adamas_icon_box_shortcode' );